 ...
 * Note: Modified to FIFO per user request! The oldest receipt is stored at the front, 
 * and the newest receipt is added to the rear.
 */
struct Receipt {
    int resident_id;
    char name[50];
...
/* 
 * DATA STRUCTURE 3: QUEUE (FIFO - First-In, First-Out)
 * Purpose: Manages the temporary line of people waiting to be processed for a transaction.
 */
struct QueueNode {
    int resident_id;
...
struct Receipt* receiptFront = NULL;       // Front of the Receipt History (FIFO)
struct Receipt* receiptRear = NULL;        // Rear of the Receipt History (FIFO)
...
void processTransaction() {
    ...
    /* 
     * ALGORITHM: ENQUEUE (Waiting Line)
    ...
    /* 
     * ALGORITHM: DEQUEUE (Waiting Line)
    ...
    /* 
     * ALGORITHM: ENQUEUE (Receipt History - FIFO)
     * Adds the generated receipt to the REAR of the history queue.
     * This guarantees the First transaction in is the First transaction out/viewed.
     */
    ...
    if (receiptRear == NULL) {
        receiptFront = receiptRear = newReceipt;
    } else {
        receiptRear->next = newReceipt;
        receiptRear = newReceipt;
    }
...
/*
 * NEW MODULE: VIEW RECEIPTS (FIFO PRINCIPLE)
 * ALGORITHM: Queue Traversal (Time Complexity: O(n))
 * Traverses the Receipt queue from Front (Oldest) to Rear (Newest).
 */
void viewReceipts() {
    printHeader(" TRANSACTION HISTORY ");
    showSpinner("Fetching Records", 800);
    
    if (receiptFront == NULL) {
        setColor(C_RED);
        ...
        setColor(C_WHITE);
        struct Receipt* temp = receiptFront;
        
        // Loop from the Front (first created) to the Rear (last created)
        while (temp != NULL) {
        ...
        setColor(C_GREEN);
        printf("\n  [INFO] Records are displayed Oldest to Newest.\n");
    }
